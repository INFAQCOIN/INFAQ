...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/INFAQCOINParams"
    else
        PARAMS_DIR="$HOME/.infaqcoin-params"
    fi
fi
...
Failed to fetch the INFAQCOIN zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
INFAQCOIN - fetch-params.sh

This script will fetch the INFAQCOIN zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common INFAQCOIN zkSNARK parameters. Note that it is
