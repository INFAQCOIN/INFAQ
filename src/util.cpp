...
const char * const INFAQCOIN_CONF_FILENAME = "infaqcoin.conf";
const char * const INFAQCOIN_PID_FILENAME = "infaqcoin.pid";
const char * const INFAQCOIN_MASTERNODE_CONF_FILENAME = "masternode.conf";


// INFAQCOIN only features
// Masternode
...
#else
    const char* pszModule = "infaqcoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\INFAQCOIN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\INFAQCOIN
// Mac: ~/Library/Application Support/INFAQCOIN
// Unix: ~/.infaqcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFAQCOIN";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\INFAQCOINParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\INFAQCOINParams
    // Mac: ~/Library/Application Support/INFAQCOINParams
    // Unix: ~/.infaqcoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "INFAQCOINParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "INFAQCOINParams";
#else
    // Unix
    return pathRet / ".infaqcoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/infaqcoin/sapling-spend.params";
        sapling_output = "/usr/share/infaqcoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/infaqcoin/sapling-spend.params";
            sapling_output = "/usr/local/share/infaqcoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", INFAQCOIN_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", INFAQCOIN_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty infaqcoin.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override infaqcoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", INFAQCOIN_PID_FILENAME));
