...
#if defined(NDEBUG)
#error "INFAQCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uiqc) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("infaqcoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> iqcInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                iqcInputs.push_back(stakeIn);
        }
        const bool hasIQCInputs = !iqcInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasIQCInputs) {
                    ...
                    for (const CTxIn& iqcIn : iqcInputs)
                        if(iqcIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasIQCInputs) {
                            for (const CTxIn& stakeIn : iqcInputs)
...
bool LoadGenesisBlock()
{
    ...
    try {
        ...
        LogPrintf(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Genesis Found : %s", block.ToString().c_str());
