...
CIqcStake* CIqcStake::NewIqcStake(const CTxIn& txin)
{
    ...
    return new CIqcStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CIqcStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CIqcStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CIqcStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CIqcStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CIqcStake::GetUniqueness() const
{
    //The unique identifier for a IQC stake is the outpoint
...
const CBlockIndex* CIqcStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CIqcStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CIqcStake::ContextCheck(int nHeight, uint32_t nTime)
{
