...
#include "qt/infaqcoin/splash.h"
#include "qt/infaqcoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("infaqcoin", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in infaqcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in infaqcoin.qrc)
...
/** Class encapsulating INFAQCOIN startup and shutdown.
...
/** Main INFAQCOIN application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (INFAQCOINGUI)
...
{
    window = new INFAQCOINGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &INFAQCOINGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &INFAQCOINGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(INFAQCOINGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(INFAQCOINGUI::DEFAULT_WALLET);
        ...
        // INFAQCOIN: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &INFAQCOINGUI::handlePaymentRequest);
        connect(window, &INFAQCOINGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. INFAQCOIN can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(infaqcoin_locale);
    Q_INIT_RESOURCE(infaqcoin);
    ...
    /// 6. Determine availability of data directory and parse infaqcoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("INFAQCOIN"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("INFAQCOIN"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // infaqcoin: links repeatedly have their payment requests routed to this process:
