...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select IQC Outputs to Spend" :
            "Select Shielded IQC to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::IQC;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show INFAQCOIN address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
