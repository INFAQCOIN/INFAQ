...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::IQC);
    ...
    if (settings.contains("nAnonymizeinfaqcoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizeinfaqcoinamount", settings.value("nAnonymizeinfaqcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in infaqcoin.cpp during shotdown to also remove the window positions
