...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter INFAQCOIN address (e.g. %1)").arg("iKrpQBdmVhyJ2RsQnosZyFHbGjcgbX41Et"));
...
{
    // return if URI is not valid or is no INFAQCOIN: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::IQC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert infaqcoin:// to infaqcoin:
    //
    //    Cannot handle this later, because infaqcoin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::IQC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "INFAQCOIN (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "INFAQCOIN (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "INFAQCOIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for INFAQCOIN*.lnk
...
{
    return GetAutostartDir() / "infaqcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a infaqcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=INFAQCOIN (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=INFAQCOIN (regtest)\n";
        else
            optionFile << "Name=INFAQCOIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the infaqcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add infaqcoin app to startup item list
