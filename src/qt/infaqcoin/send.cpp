...
#include "qt/infaqcoin/send.h"
#include "qt/infaqcoin/forms/ui_send.h"
#include "qt/infaqcoin/addnewcontactdialog.h"
#include "qt/infaqcoin/qtutils.h"
#include "qt/infaqcoin/sendchangeaddressdialog.h"
#include "qt/infaqcoin/optionbutton.h"
#include "qt/infaqcoin/sendconfirmdialog.h"
#include "qt/infaqcoin/guitransactionsutils.h"
#include "qt/infaqcoin/loadingdialog.h"
...
SendWidget::SendWidget(INFAQCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-iqc");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onIQCSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onIQCSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent IQCs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield IQCs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent IQCs to shield."));
    }
}
...
void SendWidget::onIQCSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto iqcAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(iqcAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(iqcAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(iqcAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
