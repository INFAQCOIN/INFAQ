...
#include "qt/infaqcoin/forms/ui_addresseswidget.h"
#include "qt/infaqcoin/addresslabelrow.h"
#include "qt/infaqcoin/addnewaddressdialog.h"
#include "qt/infaqcoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(INFAQCOINGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto iqcAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(iqcAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(iqcAdd);
        if (walletModel->updateAddressBookLabels(iqcAdd, label.toUtf8().constData(),
