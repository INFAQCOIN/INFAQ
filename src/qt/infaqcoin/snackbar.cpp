...
#include "qt/infaqcoin/forms/ui_snackbar.h"
#include "qt/infaqcoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(INFAQCOINGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &INFAQCOINGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
