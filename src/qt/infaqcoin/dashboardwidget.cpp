...
#include "qt/infaqcoin/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/infaqcoin/forms/ui_dashboardwidget.h"
#include "qt/infaqcoin/qtutils.h"
#include "qt/infaqcoin/sendconfirmdialog.h"
#include "qt/infaqcoin/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(INFAQCOINGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareIqc, "square-chart-iqc");
    setCssProperty(ui->labelIqc, "text-chart-iqc");
    ...
    setCssProperty(ui->labelAmountIqc, "text-stake-iqc-disable");
    ...
    if (window)
        connect(window, &INFAQCOINGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("IQC")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair IQC
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            iqc = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalIqc += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesIqc.append(iqc);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(54, 145, 33));
    ...
    set0->append(chartData->valuesIqc);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalIqc > 0) {
        setCssProperty(ui->labelAmountIqc, "text-stake-iqc");
    } else {
        setCssProperty(ui->labelAmountIqc, "text-stake-iqc-disable");
    }
    forceUpdateStyle({ui->labelAmountIqc});
    ui->labelAmountIqc->setText(GUIUtil::formatBalance(chartData->totalIqc, nDisplayUnit));
